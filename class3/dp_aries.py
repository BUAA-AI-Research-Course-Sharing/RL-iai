...
def p_state_reward(state, action):
    ...
    if action == 0:
        #Fill and submit this code
        if state - 21 in diamond_block or state - 21 == start:
            return (1,state-21,-1)
        elif state - 21 == end:
            return (1,state-21,100)
        else:
            return (1,state,-1)
    ...
    if action == 2:
         #Fill and submit this code
        if state - 1 in diamond_block or state - 1 == start:
            return (1,state-1,-1)
        elif state - 1 == end:
            return (1,state-1,100)
        else:
            return (1,state,-1)
...
def compute_value_function(policy, gamma):
    ...
    ## 遍历所有状态
        ...
        ## 返回当前状态下执行动作得到的转移概率、下一状态和奖励
            trans_pro,next_state,reward = p_state_reward(state,action)
        ## 计算策略下状态价值
            value_table[states.index(state)] = reward + gamma * trans_pro * value_table[states.index(next_state)]
    ## 价值表前后两次更新之差小于阈值时停止循环
        if np.fabs(new_value_table - value_table).max() < threshold:
            break
    #Fill and submit this code
    return value_table
...
def next_best_policy(value_table, gamma):
    ...
    for state in states:
        ...
        ## 返回当前状态-动作下一步的状态、转移概率和奖励
            trans_pro,next_state,reward = p_state_reward(state,action)
            ## 计算当前状态下执行当前动作的价值
            action_table[action] = reward + trans_pro * gamma * value_table[states.index(next_state)]
        ## 策略提升：选取动作值最大的动作更新策略
        better_policy.append(np.argmax(action_table))
    #Fill and submit this code
