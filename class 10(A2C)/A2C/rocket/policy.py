...
def calculate_returns(next_value, rewards, masks, gamma=0.99):
    ...
    for step in reversed(range(len(rewards))):
        ...
        returns.insert(0, R)
#         print(step, R, rewards[step],masks[step])
    return returns
...
class ActorCritic(nn.Module):
    ...
    def __init__(self, input_dim, output_dim): # 8, 9
        super().__init__()
    ...
    def forward(self, x):
        ...
        # *** add your code here ***
        # **************************
        # probs = softmax(actor(x))
        # value = critic(x)
        probs = self.softmax(self.actor(x))
        value = self.critic(x) 
...
#         print('get_action',state)
        ...
        probs, value = self.forward(state) # dimension: 9, 1
#         print('\n===',probs,value,'\n===')
        probs = probs[0, :]
        value = value[0]
...
#         print(action_id, log_prob, value)
    ...
    def update_ac(network, rewards, log_probs, values, masks, Qval, gamma=0.99):
        ...
        # **************************
        # advantage = Q - v
        # actor_loss = E[-log(probs)*advantage]
        # critic_loss = E[(1/2)*advantage^2]
